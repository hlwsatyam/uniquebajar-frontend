  ...
  const fetchProductData = async () => {
    try {
      const response = await axios.get(
        `${BaseApiUrl}/api/data/singlef/${productId}`
      );
      if (response.status === 200) {
        const allImage = JSON.parse(response.data.image_urls).map(
          (image) => `${BaseApiUrl}/uploads/${image}`
        );
        ...
        setFormData({
          ...
          real_price: response.data?.real_price,
          discount_price: response.data.discount_price,
          feature_details: JSON.parse(response.data.feature_details),
          category: response.data.category_name,
          subcategory: response.data.subcategory_name,
          description: response.data.description,
          product_informations: JSON.parse(response.data.product_informations),
          return_time: response.data.return_time,
          images: allImage,
        });
      }
    } catch (error) {
